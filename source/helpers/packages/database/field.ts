...
export const SystemFields: {
  ...
  updated: {
    user: {
      name: 'user_updated',
      builder: async (t) => {
        t.uuid('user_updated').references('id').inTable('directus_users').onDelete('SET NULL')
      },
      presentation: {
        hidden: true,
        width: 'half',
...
export const addFieldToDirectusAdmin = async (
  ...
  await knex('directus_fields').insert({
    collection: table,
    field: field.name,
    special: field.config!.special!.length == 0 ? null : field.config!.special!.join(','),
    interface: field.presentation!.interface!.type,
    options:
      field.presentation!.interface!.options == null
        ? null
        : JSON.stringify(field.presentation!.interface!.options),
    display: field.presentation!.display!.type,
    display_options:
      field.presentation!.display!.options == null
        ? null
