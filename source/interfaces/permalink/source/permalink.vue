<template>
	...
	>
		<template #prepend>
			...
			<span style="direction: rtl;">
				<span class="permalink prefix">{{ prefix }}</span><span class="permalink slash">/</span>
			</span>
		</template>
		...
		<template #append>
			...
			<v-icon class="action metric" :class="quality.rating" @click="metricPanelActive = true" name="speed" />
		</template>
	</v-input>
	...
	<v-dialog v-model="helpPanelActive">
		<v-card>
			...
			<v-card-text>
				This is some information about permalinks.
			</v-card-text>
		</v-card>
	</v-dialog>
	...
	<v-dialog v-model="metricPanelActive">
		<v-card>
			...
			<v-card-text>
				This is some information about how the slug might perform.
			</v-card-text>
			<v-card-text>
				{{ quality }}
			</v-card-text>
...
import { stopwords } from './stopwords';
	...
	const quality = computed((): {
		...
		words.forEach((word: string) => {
			...
			if (parseFloat(word) != NaN) score--

			let year = new Date().getFullYear()

			// * Rule: Should not have a recent year in the slug
			if ( // This will compound on the negative score above. This is intentional.
				parseFloat(word) != NaN
			...
			if (parseFloat(word) != NaN && parseFloat(word) == year) score--
...
<style scoped>
	...
	.v-icon.metric.poor { color: var(--danger); }
	.v-icon.metric.poor:hover { color: var(--danger-150); }
	.v-icon.metric.average { color: var(--warning); }
	.v-icon.metric.average:hover { color: var(--warning-150); }
	.v-icon.metric.good { color: var(--success); }
	.v-icon.metric.good:hover { color: var(--success-150); }
</style>
